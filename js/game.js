...
function Start() {
    ...
    var toadd = fivepoint + fifteenpoint + twentyfivepoint;
    fivepoint += (ballsnums - toadd);
    var pacman_remain = 1;
    var numGn = numGhost;
    lifeRemaining = 3;
    document.getElementById("life").src = "images/life3.png";
    ...
    intervalCherry = setInterval(UpdatePositionCherry, 2500);
...
function UpdatePositionCherry() {
    if (cherry != null) {
        ...
        while (bool) {
            ...
            if (rand == 1) {
                if (i + 1 >= 0 && i + 1 <= 15 && board[i + 1][j] !== 4 && board[i + 1][j] != 8 && board[i + 1][j] != 9 && board[i + 1][j] != 10 && board[i + 1][j] != 2) {
                ...
                if (j - 1 >= 0 && j - 1 <= 11 && board[i][j - 1] !== 4 && board[i][j - 1] !== 8 && board[i][j - 1] !== 9 && board[i][j - 1] !== 10 && board[i][j - 1] !== 2) {
                    board[i][j] = lastPosChherry;
...
function endGame(vers) {
    ...
    if (vers == 0) {
        if (showMass == false) {
            alert("Your lives are over.. you lost");   //life is over
            showMass = true;
            window.clearInterval(interval);
